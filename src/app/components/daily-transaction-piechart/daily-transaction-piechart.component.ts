...
export class DailyTransactionPiechartComponent implements OnInit {
  ...
  updateChart() {
	...
	// 실제 값 집계
	const typeData: { [label: string]: number } = {
	  入金: 0,
	  出金: 0,
	  '振込(出金)': 0,
	  '振込(入金)': 0,
	...
	selectedDateTransactions.forEach((transaction: TransactionComponent) => {
	    let type = transaction.transactionType.slice(5); // e.g., 'Deposit'
	    type = type.charAt(0).toUpperCase() + type.slice(1).toLowerCase(); // PascalCase
	    ...
	    typeData[label] += transaction.amount;
	  });
